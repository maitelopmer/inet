...
void RIPRouting::initialize(int stage)
{
    if (stage == 0) {
        ...
        addressPolicy = rt->getRouterIdAsGeneric().getAddressPolicy();
        ...
        for (InterfaceVector::iterator it = ripInterfaces.begin(); it != ripInterfaces.end(); ++it)
            socket.joinMulticastGroup(addressPolicy->getLinkLocalRIPRoutersMulticastAddress(), it->ie->getInterfaceId());
...
void RIPRouting::sendInitialRequests()
{
    for (InterfaceVector::iterator it = ripInterfaces.begin(); it != ripInterfaces.end(); ++it)
    {
        ...
        sendPacket(packet, addressPolicy->getLinkLocalRIPRoutersMulticastAddress(), ripUdpPort, it->ie);
    }
}
...
void RIPRouting::processUpdate(bool triggered)
{
    ...
    for (InterfaceVector::iterator it = ripInterfaces.begin(); it != ripInterfaces.end(); ++it)
        sendRoutes(addressPolicy->getLinkLocalRIPRoutersMulticastAddress(), ripUdpPort, *it, triggered);
...
void RIPRouting::sendRoutes(const Address &address, int port, const RIPInterfaceEntry &ripInterface, bool changedOnly)
{
    ...
    for (RouteVector::iterator it = ripRoutes.begin(); it != ripRoutes.end(); ++it)
    {
        ...
        entry.nextHop = addressPolicy->getUnspecifiedAddress(); //route->getNextHop() if local ?
