...
void RIPRouting::sendInitialRequests()
{
    for (InterfaceVector::iterator it = ripInterfaces.begin(); it != ripInterfaces.end(); ++it)
    {
        ...
        sendPacket(packet, addressPolicy->getLinkLocalRIPRoutersMulticastAddress(), RIP_UDP_PORT, it->ie);
    }
}
...
void RIPRouting::processTriggeredUpdate()
{
    ...
    for (InterfaceVector::iterator it = ripInterfaces.begin(); it != ripInterfaces.end(); ++it)
        sendRoutes(addressPolicy->getLinkLocalRIPRoutersMulticastAddress(), RIP_UDP_PORT, *it, true);
