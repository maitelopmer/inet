...
#define coreEV (ev.isDisabled()||!coreDebug) ? ev : ev << "IdealChannelModel: "
...
void IdealChannelModel::initialize()
{
    ...
    coreEV << "initializing IdealChannelModel\n";
...
void IdealChannelModel::unregisterRadio(RadioEntry *r)
{
    ...
    for (RadioList::iterator it = radios.begin(); it != radios.end(); ++it)
    {
        if (it->radioModule == r->radioModule)
        {
            ...
            radios.erase(it);
            maxTransmissionRange = -1.0;    // invalid value
            return;
        }
    }
...
void IdealChannelModel::sendToChannel(RadioEntry *srcRadio, IdealAirFrame *airFrame)
{
    ...
    double sqrTransmissionRange = maxTransmissionRange * maxTransmissionRange;
    ...
    for (RadioList::iterator it=radios.begin(); it !=radios.end(); ++it)
    {
        ...
        {
            coreEV << "skip sending message to radio, radio is out of max. transmission range\n";
        }
    }
    delete airFrame;
}
