...
void IdealRadio::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        refreshCoverageInterval = 0;
        if (this->hasPar("refreshCoverageInterval"))
            refreshCoverageInterval = par("refreshCoverageInterval");
    }
    else if (stage == 2)
    {
...
IdealRadio::~IdealRadio()
{
    cancelAndDelete(updateDisplayStringTimer);
}
...
IdealAirFrame *IdealRadio::encapsulatePacket(cPacket *frame)
{
    ...
    << " will be transmitted at " << (bitrate/1e6) << "Mbps\n";
    return airframe;
}
...
void IdealRadio::handleSelfMsg(cMessage *msg)
{
    ...
    if (msg == updateDisplayStringTimer)
    {
        updateDisplayString();
    }
    ...
    else if (msg->getKind() == MK_TRANSMISSION_OVER)
    {
        ...
        setRadioState(); // now the radio changes the state and sends the signal
...
{
    // calculate distance
    const Coord& framePos = airframe->getTransmissionStartPosition();
    double distance = getRadioPosition().distance(framePos);

    if (distance > airframe->getTransmissionRange())
    {
        // out of range, drop
        delete airframe;
        return;
...
void IdealRadio::setRadioState()
{
    RadioState::State newState = isEnabled()
            ? (onTransmit ? RadioState::TRANSMIT : ((numReceives>0) ? RadioState::RECV : RadioState::IDLE))
            : RadioState::SLEEP;
    if (rs != newState)
    {
        rs = newState;
...
void IdealRadio::updateDisplayString()
{
    ...
    if (updateDisplayStringTimer==NULL && refreshCoverageInterval>0)
        updateDisplayStringTimer = new cMessage("refresh timer");
    if (refreshCoverageInterval>0)
        scheduleAt(simTime()+refreshCoverageInterval, updateDisplayStringTimer);
}
